        ...
        <Wrapper className="pt-24" style={{ background: "url('./pictures/authpageBackground.png') 1% 50% rgba(255,255,255,0.1)" }}>
            ...
            <div className="md:flex justify-between">
                <div className="mt-8 md:w-6/12">
                    ...
                    <div className="my-8 bg-qosgray bg-opacity-90 grid grid-flow-row gap-y-2 max-w-sm lg:max-w-lg lg:mx-0 mx-auto h-96 rounded-xl shadow-sm p-4">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-8 gap-y-4 my-8 mx-auto">
                {media.Logos.customers.map((Partner, index) => <div className="p-2 bg-coolGray-100 bg-opacity-80 shadow-lg w-11/12 rounded-lg mx-auto"> <img key={`customer-${index}`} className="mx-auto" src={Partner} alt="hello part" /></div> )}
            </div>
            <h3 className="my-8 mt-20 text-center"> {Infos['Invitation'].Title} </h3>
            <Link to="/contact-us"><button className="btn btn-primary bg-qosorange rounded-full btn-md my-8 block mx-auto">{Infos['Invitation'].buttonText}<span class="fi fi-rr-arrow-small-right "></span></button></Link>
