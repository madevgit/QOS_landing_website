...
export default function PricingPage() {
    ...
    return (
        <Wrapper className="mt-24" style={{ background: "url('./pictures/authpageBackground.png')", backgroundSize: "100% 0.4%" }}>
            ...
            <h4 class="mt-4 text-qosdark text-opacity-80 font-normal md:mt-8"> {Infos.description} </h4>
            <div className="mt-10 md:flex flex-wrap justify-between lg:w-11/12 mx-auto">
                <div class="pricingCard h-96 mx-auto lg:mx-0 lg:max-w-sm max-w-xs p-4 border border-opacity-75 border-qosgray">
                    ...
                    <p className="w-11/12 mx-auto text-center">{Infos.pricingCard.fees}</p>
            ...
            <section className="relative overflow-hidden md:overflow-visible justify-between mx-auto max-w-lg md:max-w-screen-lg flex">
                <div className={`min-w-full md:w-6/12 mr-8 md:min-w-min pricingCard rounded-r-lg rounded-b-lg md:rounded-lg grid grid-flow-row gap-y-4 transform bg-opacity-70 ${isAboutAPI ? 'translate-x-full md:translate-x-0 md:scale-x-100 scale-x-0' : 'translate-x-0 scale-x-100'} origin-left duration-500 ease bg-qosgray p-4 mb-4`}>
                    <h3 className="text-center my-4 hidden md:block">{Infos["details"].USSD.Title}</h3>
                    {Infos["details"].USSD.Lists.map(item => <DetailsItem description={item} />)}
                ...
                <div className={`min-w-full md:w-6/12 ml-8 pricingCard md:min-w-min rounded-r-lg rounded-b-lg md:rounded-lg bg-qosgray bg-opacity-70 transform  ${isAboutAPI ? '-translate-x-full md:translate-x-0 scale-x-100 rounded-lg' : 'translate-x-0 md:scale-x-100 scale-x-0'} origin-left duration-500 ease  p-4 mb-4`}>
                    <div className="grid grid-flow-row gap-y-4">
                        <h3 className="text-center my-4 hidden md:block">{Infos["details"].API.Title}</h3>
                        {Infos["details"].API.Lists.map(item => <DetailsItem description={item} />)}
                        <div className="w-full">
