    ...
    <div className="md:flex justify-between w-full mx-auto">
      ...
      <section className="md:w-7/12 min-h-screen">
        ...
        <NavsTabs
          ...
          mainStyle={`p-2 flex ${
            stickyDocsHeader && "fixed bg-qosgray z-50 top-16"
          } w-full left-0 justify-around mb-5 text-mmd md:text-dmd`}
        />
      </section>
    </div>
  );
    ...
    >
      <div className="flex justify-between h-13 mx-auto">
        ...
        >
          ...
          />
          <h4 className="text-qosorange md:text-dH4 mx-auto max-w-max block">
            Docs
          </h4>
        </Link>
...
export function ApiDocs({ Open, setOpen }) {
  ...
  return (
    <section className="md:flex">
      <nav
        className={`h-screen lg:w-80 transform md:border-r border-qosdark border-opacity-20 transition-all md:transform-none duration-500 md:duration-75 fixed md:left-0 md:right-auto px-6 bg-qosgray md:bg-opacity-0 top-0 right-0 z-20 ${
      ...
      >
        <ul className="mt-24 md:mt-32">
          <Link
            to={`${path}`}
            className="docslinks my-2 font-medium text-opacity-70 text-qosdark"
      ...
      <div className="max-h-full max-w-screen-md overflow-y-auto overflow-x-hidden hideScroll">
        <Switch>
          <Route exact path={`${path}`}>
            <div id="Design">
              ...
              <div className="">
                <div>
                  <p className="">
                    A MobilePayment© transaction provides Mobile Money
                    capability application programming interfaces (APIs) for
                    third-party applications (App for short) to connect to it
                    and use its Mobile Money capability for Bill Payment,
                    Depositing Money in Subscribers account & Querying
                    Subscriber’s account in a third-party system. The subsequent
                    sections show the message structure for Mobile Payment
                  </p>
                </div>
                <div className="mt-4">
                ...
                <div className="">
                  ...
                  <div>
                    <NavsTabs
                      ...
                      mainStyle={`p-2 flex mt-4 w-full left-0 justify-around mb-5 text-mmd md:text-dmd`}
                    />
                  </div>
                </div>
              </div>
          ...
          <Route exact path={`${path}/auth`}>
            <div id="auth">
              ...
              <p className="mb-4">
                ...
                into the HTTP protocol. The client sends HTTP requests with the
                Authorization header that contains the word Basic word followed
                by a space and a base64-encoded string username:password. For
          ...
          <Route exact path={`${path}/security`}>
            <div id="security">
              <p className="">
                Requests will be sent over <CodeBlock value="HTTPS" /> only.
                Each request is also sent using a Basic Authentication. With
                Basic Authentication, clients send its Base64 encoded
                credentials with each request, using HTTP [Authorization]
                header. That means each request is independent of other request
                and server may/does not maintain any state information for the
                client.
              </p>
            </div>
          </Route>
            ...
            <div id="payment">
              <div className="">
                ...
                <p className="my-4">
                  ...
                  subscribers account. Partners must code the App based on the
                  API field requirements so that the App can send correct
          ...
          <Route exact path={`${path}/transaction`}>
            <div id="transaction">
              <div className="">
                ...
                <p>
                  A Get Transaction Status message is a request to get the
                  details of an already processed transaction.
                </p>
                <div className="mt-4">
                  <span className="font-mono font-bold block text-mmd">
                    Endpoint URI:
