...
export default function Navbar() {
    ...
    return (
        <Wrapper childrenClassName={`flex h-8 md:h-10 ${!dark ? 'text-qosgray' : 'text-qosdark'} text-opacity-70 items-center justify-between`} className={`z-50 top-0 ${background && 'bg-qosgray'} bg-opacity-80 rounded-none py-4 fixed w-full`} >
            <Link to="/">
            ...
            <nav id="navbarLink" className="w-6/12 max-h-full">
                <ul className="hidden w-full text-mmd items-center justify-around text-opacity-80 md:flex">
                    <li className="relative group">
                        <Link id="product" to="#">Products
                            <span className="fi fi-rr-angle-small-down align-bottom inline-block text-msm ml-1"></span>
                        </Link>
                        <nav id="products" className="bg-qosgray z-0 -ml-10 scale-y-0 origin-bottom -mt-24 group-hover:mt-0 opacity-0 group-hover:opacity-100 group-hover:scale-y-100 transform ease-out  duration-500 rounded-md px-4 absolute p-2 pt-5 min-w-max text-qosdark">
                            <LinkGroup>
                    ...
                    <li className="relative group">
                        ...
                        <nav className="bg-qosgray z-0 -ml-10 scale-y-0 origin-bottom -mt-12 group-hover:mt-0 opacity-0 group-hover:opacity-100 group-hover:scale-y-100 transform ease-out  duration-500 rounded-md px-4 absolute p-2 pt-5 min-w-max text-qosdark">
                            <LinkGroup>
                    ...
                    <li>
                        <Link id="developer" to="/docs">Developers</Link>
                    </li>
                    <li>
            ...
            <div className="hidden md:block">
                ...
                <Link to="/register" className="p-2 inline-block rounded-full px-3 bg-opacity-90 shadow-lg bg-gradient-to-bl from-qosorange to-qosblue text-qosgray font-medium btn-sm max-w-max mx-auto m-2" >
                    Create Acocount
                </Link>
            </div>
            <MobileNavbar />
